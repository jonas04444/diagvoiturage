    ...
    def dessiner_service(self):
        ...
        for idx_ligne, ligne in enumerate(lignes_y):
            ...
            for v in ligne:
                ...
                print(
                    f"  L{idx_ligne + 1} V{v.num_voyage}: {h_d}-{h_f} ({v.hdebut}-{v.hfin}min) → x1={x1:.1f}, x2={x2:.1f} y={y_rect}")
...
class Tab5CreationManuelle(ctk.CTkFrame):
    ...
    def ajouter_voyages_au_service(self):
        ...
        for item in self.tree_voyages.get_children():
            ...
            if values[0] == '☑':
                ...
                print(
                    f"🔍 Sélectionné : idx={idx}, V{v.num_voyage} {v.num_ligne} {voyage.minutes_to_time(v.hdebut)}-{voyage.minutes_to_time(v.hfin)} {v.arret_debut}→{v.arret_fin}")
    ...
    def completer_avec_solveur(self):
        ...
        ctk.CTkLabel(
            dialog, text="🤖 Complétion automatique (Algorithme glouton)",
            font=("Arial", 16, "bold")
        ).pack(pady=15)

        info_text = f"📊 Services existants : {len(self.services)}\n"
        info_text += f"📋 Voyages non assignés : {len(voyages_non_assignes)}\n\n"
        info_text += "🔧 Algorithme glouton :\n"
        info_text += "  • Prend les voyages dans l'ordre chronologique\n"
        info_text += "  • Pour chaque service, essaie d'ajouter les voyages compatibles\n"
        info_text += "  • Respecte les contraintes horaires et le battement\n"
        info_text += "  • Utilise votre fonction voyages_compatibles()\n\n"
        info_text += "💡 Rapide mais pas optimal (pas OR-Tools)"
        ...
        def lancer():
            try:
                ...
                msgbox.showerror("Erreur", "Valeurs invalides pour les battements")
    ...
    def _executer_completion(self, voyages_non_assignes, battement_min, battement_max, verifier_arrets):
        """Exécute la complétion avec algorithme glouton"""

        print("\n" + "=" * 70)
        print("🤖 COMPLÉTION AUTOMATIQUE (Algorithme glouton)")
        print("=" * 70)
        print(f"⚙️ Paramètres :")
        print(f"   Battement min : {battement_min} min")
        print(f"   Battement max : {battement_max if battement_max else 'Aucune limite'}")
        print(f"   Vérifier arrêts : {'Oui' if verifier_arrets else 'Non'}")
        ...
        for service in self.services:
            ...
            if h_debut and h_fin:
                ...
                print(f"   ⏰ Pas de contraintes horaires")
            ...
            for v in sorted(voyages_restants, key=lambda x: x.hdebut):
                ...
                for v_exist in voyages_service:
                    ...
                    # Vérifier battement et arrêts avec votre fonction
                    if v.hfin <= v_exist.hdebut:
                        if not voyages_compatibles(v, v_exist, voyages_service, battement_min, battement_max,
                                                   verifier_arrets):
                            compatible = False
                            break
                    elif v_exist.hfin <= v.hdebut:
                        if not voyages_compatibles(v_exist, v, voyages_service, battement_min, battement_max,
                                                   verifier_arrets):
                            compatible = False
                            break
                ...
                if compatible:
                    ...
                    print(
                        f"   ✅ V{v.num_voyage} ajouté ({voyage.minutes_to_time(v.hdebut)}-{voyage.minutes_to_time(v.hfin)})")
