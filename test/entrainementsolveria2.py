...
class service_agent:
    ...
    def duree_services_maximum(self, duree_maximum):
        ...
        if duree > duree_maximum:
            return False
        return True  # Correction: retourner True si la durée est valide
...
def solvertest(listes, battement_minimum, verifier_arrets=True, max_solutions=10, max_service_choisi=True,
    ...
    """
    Résout le problème d'affectation des voyages aux services.

    Args:
        listes: Liste des voyages à affecter
        battement_minimum: Temps minimum entre deux voyages (en minutes)
        verifier_arrets: Si True, vérifie la compatibilité des arrêts
        max_solutions: Nombre maximum de solutions à trouver
        max_service_choisi: Nombre maximum de services (True pour utiliser le nombre de voyages)
        duree_max_service: Durée maximale d'un service en minutes (par défaut 540 = 9h)
    """
    ...
    # === NOUVELLE SECTION: Contrainte de durée maximale de service ===
    # Pour chaque service possible, on crée des variables pour suivre le début et la fin
    debut_service = [model.NewIntVar(0, 24 * 60, f"debut_service_{s}") for s in range(max_services)]
    fin_service = [model.NewIntVar(0, 24 * 60, f"fin_service_{s}") for s in range(max_services)]

    # Variables booléennes pour savoir si un voyage est affecté à un service
    affectation = {}
    for i in range(n):
        for s in range(max_services):
            affectation[(i, s)] = model.NewBoolVar(f"voyage_{i}_service_{s}")
            # Lier la variable booléenne à l'affectation réelle
            model.Add(service[i] == s).OnlyEnforceIf(affectation[(i, s)])
            model.Add(service[i] != s).OnlyEnforceIf(affectation[(i, s)].Not())

    # Pour chaque service, calculer le début minimum et la fin maximum
    for s in range(max_services):
        # Variables pour savoir si le service est utilisé
        service_utilise = model.NewBoolVar(f"service_{s}_utilise")

        # Un service est utilisé si au moins un voyage lui est affecté
        ...
        # Pour chaque voyage, si affecté à ce service, le début du service doit être <= début du voyage
        for i in range(n):
        ...
        # Contrainte: durée du service <= duree_max_service
        duree_service = model.NewIntVar(0, 24 * 60, f"duree_service_{s}")
    ...
    for sol_idx, solution in enumerate(solution_collector.solutions, 1):
        ...
        for num_service in sorted(services_dict.keys()):
            ...
            duree = nouveau_service.duree_services()
            if duree > duree_max_service:
                print(
                    f"⚠️  ATTENTION: Service {num_service} dépasse la durée maximale: {duree} min (max: {duree_max_service} min)")
                continue
...
if __name__ == "__main__":
    ...
    # Test avec durée maximale de 9 heures (540 minutes)
    print("=" * 70)
    print("TEST AVEC DURÉE MAXIMALE DE SERVICE: 9h (540 minutes)")
    print("=" * 70)
    ...
    for idx, services in enumerate(solutions, 1):
        ...
        for s in services:
            print(s)
            print(f"  → Durée totale: {s.duree_services()} minutes (max autorisé: 540 min)")
            if not s.duree_services_maximum(540):
                print(f"  ⚠️  ERREUR: Ce service dépasse la durée maximale!")