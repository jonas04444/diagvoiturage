...
def solvertest(listes, battement_minimum, battement_maximum = 50, verifier_arrets=True, max_solutions = 10,
    ...
    # LIGNE SUPPRIMÉE : model.Minimize(sum(service_utilise))
    ...
    for sol in collector.solutions:
        ...
        for s, vs in services.items():
            # Trier les voyages chronologiquement
            voyages_ordonnes = sorted(vs, key=lambda v: v.hdebut)

            debut = min(v.hdebut for v in voyages_ordonnes)
            type_service = "matin" if debut < heure_debut_apres_midi else "apres_midi"
            sa = service_agent(s, type_service)
            for v in voyages_ordonnes:
                sa.ajout_voyages(v)
            resultat.append(sa)
