"""
TAB 5 - VERSION AVEC VRAI SOLVEUR OR-TOOLS
- Utilise gestion_voiture.py pour l'optimisation
...
from gestion_voiture import optimiser_affectation  # ✅ NOUVEAU : Import du vrai solveur
import csv
...
class TimelineVisuelle(ctk.CTkFrame):
    ...
    def dessiner_service(self):
        ...
        # Organiser les voyages en lignes pour éviter les chevauchements
...
class ServiceCard(ctk.CTkFrame):
    ...
    def __init__(self, parent, service, on_delete=None, on_select=None, on_edit_constraints=None, **kwargs):
        ...
        self.on_edit_constraints = on_edit_constraints
...
class Tab5CreationManuelle(ctk.CTkFrame):
    """Frame pour le Tab 5 - Création manuelle de services avec OR-Tools"""

    def __init__(self, parent):
        super().__init__(parent)

        self.voyages_disponibles = []
        self.voyages_disponibles_tries = []
        self.services = []
    ...
    def creer_interface(self):
        ...
        # ✅ NOUVEAU : Bouton avec vrai OR-Tools
    ...
    def creer_nouveau_service(self):
        """Crée un service avec contraintes horaires"""
    ...
    def ajouter_voyages_au_service(self):
        ...
        msgbox.showinfo("Succès", f"{len(voyages_a_ajouter)} voyage(s) ajouté(s)")
    ...
    def supprimer_voyage_du_service(self, voyage_obj):
        """Supprime un voyage du service sélectionné"""
        if not self.service_selectionne:
            return
    ...
    def editer_contraintes(self, service):
        """Éditer les contraintes horaires d'un service"""
        dialog = ctk.CTkToplevel(self)
    ...
    def completer_avec_ortools(self):
        ...
        ctk.CTkLabel(
            dialog, text="🤖 Optimisation avec OR-Tools CP-SAT",
            font=("Arial", 18, "bold")
        ).pack(pady=15)

        info_text = f"📊 Services existants : {len(self.services)}\n"
        info_text += f"📋 Voyages non assignés : {len(voyages_non_assignes)}\n\n"
        info_text += "🔧 Algorithme OR-Tools CP-SAT :\n"
        info_text += "  • Résolution optimale du problème d'affectation\n"
        info_text += "  • Maximise le nombre de voyages assignés\n"
        info_text += "  • Respecte toutes les contraintes :\n"
        info_text += "    - Contraintes horaires des services\n"
        info_text += "    - Battement min/max entre voyages\n"
        info_text += "    - Compatibilité des arrêts\n"
        info_text += "    - Non-chevauchement temporel\n\n"
        info_text += "💡 Beaucoup plus puissant que l'algorithme glouton"
        ...
        # Battement minimum
        ctk.CTkLabel(frame_config, text="⏱️ Battement minimum (min) :", font=("Arial", 11)).pack(pady=5)
        entry_battement_min = ctk.CTkEntry(frame_config, width=200, height=35)
        entry_battement_min.insert(0, "5")
        entry_battement_min.pack(pady=5)

        # Battement maximum
        ctk.CTkLabel(frame_config, text="⏱️ Battement maximum (min) :", font=("Arial", 11)).pack(pady=5)
        entry_battement_max = ctk.CTkEntry(frame_config, width=200, height=35, 
                                          placeholder_text="Laissez vide = aucune limite")
        entry_battement_max.insert(0, "50")
        entry_battement_max.pack(pady=5)

        # Vérifier arrêts
        ctk.CTkLabel(frame_config, text="🚏 Vérifier les arrêts :", font=("Arial", 11)).pack(pady=5)
        check_arrets = ctk.CTkCheckBox(frame_config, text="Activer (recommandé)", font=("Arial", 11))
        check_arrets.select()
        check_arrets.pack(pady=5)

        # Temps limite
        ctk.CTkLabel(frame_config, text="⏱️ Temps limite (secondes) :", font=("Arial", 11)).pack(pady=5)
        entry_temps = ctk.CTkEntry(frame_config, width=200, height=35)
        entry_temps.insert(0, "60")
        entry_temps.pack(pady=5)

        def lancer():
            try:
                battement_min = int(entry_battement_min.get())
                battement_max_str = entry_battement_max.get().strip()
                battement_max = int(battement_max_str) if battement_max_str else None
                verifier = check_arrets.get() == 1
                temps_limite = int(entry_temps.get())
                
                dialog.destroy()
                self._executer_ortools(voyages_non_assignes, battement_min, battement_max, verifier, temps_limite)
            except ValueError:
                msgbox.showerror("Erreur", "Valeurs invalides pour les paramètres")

        ctk.CTkButton(
            dialog, text="🚀 Lancer l'optimisation OR-Tools",
            command=lancer, height=50,
            fg_color="#4CAF50", hover_color="#388E3C",
            font=("Arial", 14, "bold")
        ).pack(pady=20)

    def _executer_ortools(self, voyages_non_assignes, battement_min, battement_max, verifier_arrets, temps_limite):
        """✅ NOUVEAU : Exécute l'optimisation OR-Tools"""
        
        try:
            # Importer et lancer l'optimisation
            ...
            if success:
                # Mettre à jour le tracking des voyages assignés
                ...
                # Message de succès
                nb_affectes = resultats['nb_affectes']
                nb_restants = len(voyages_non_assignes) - nb_affectes
                temps = resultats['temps']
                
                if nb_restants > 0:
                    msg = f"✅ Optimisation terminée !\n\n"
                    msg += f"📊 {nb_affectes} voyage(s) affecté(s)\n"
                    msg += f"⚠️ {nb_restants} voyage(s) non assignable(s)\n"
                    msg += f"⏱️ Temps: {temps:.2f}s"
                else:
                    msg = f"✅ Optimisation parfaite !\n\n"
                    msg += f"📊 Tous les {nb_affectes} voyages ont été affectés\n"
                    msg += f"⏱️ Temps: {temps:.2f}s"
                
                msgbox.showinfo("Résultat OR-Tools", msg)
            else:
                msgbox.showerror(
                    "Erreur",
                    "OR-Tools n'a pas trouvé de solution.\n"
                    "Essayez d'assouplir les contraintes ou d'ajouter plus de services."
                )
                
        except ImportError:
            msgbox.showerror(
                "Erreur",
                "Module gestion_voiture non trouvé.\n"
                "Assurez-vous que gestion_voiture.py est dans le même dossier."
            )
...
if __name__ == "__main__":
    app = ctk.CTk()
    app.title("Test Tab 5 - Avec OR-Tools")
    app.geometry("2000x1125")
